...
export default function CreateInvoicePage() {
  ...
  const generateInvoiceNumber = async () => {
    try {
      const response = await fetch('/api/admin/invoices/generate-number');
      if (response.ok) {
        const data = await response.json();
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              Generate a new invoice for diamond sales
            </p>
          </div>
              ...
              <CardContent className="grid gap-6">
                ...
                {selectedMaster && (
                  <div className="p-4 bg-muted rounded-lg space-y-2">
                    <h4 className="font-medium">Selected Company Details:</h4>
                    <p className="text-sm">{selectedMaster.companyName}</p>
                    <p className="text-sm">{selectedMaster.addressLine1}</p>
                    {selectedMaster.addressLine2 && (
                      <p className="text-sm">{selectedMaster.addressLine2}</p>
                    )}
                    <p className="text-sm">
                      {selectedMaster.city}, {selectedMaster.state} {selectedMaster.postalCode}
                    </p>
                    <p className="text-sm">{selectedMaster.country}</p>
                  </div>
                )}
              </CardContent>
            </Card>
                  ...
                  <div key={index} className="p-4 border rounded-lg space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          value={item.clarity}
                          onChange={(e) => handleItemChange(index, "clarity", e.target.value)}
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          value={item.reportNo}
                          onChange={(e) => handleItemChange(index, "reportNo", e.target.value)}
                        />
                      </div>
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) => setFormData(prev => ({ ...prev, shipmentCost: e.target.value }))}
                    />
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      type="number"
                      step="0.01"
                      value={formData.crPayment}
                      onChange={(e) => setFormData(prev => ({ ...prev, crPayment: e.target.value }))}
                    />
                  </div>
                </div>
