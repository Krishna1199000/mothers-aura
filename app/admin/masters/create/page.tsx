  ...
  const fetchUsers = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const employeeUsers = users.filter((user: User) => user.role === 'EMPLOYEE');
        
        setAdmins(adminUsers);
        setEmployees(employeeUsers);

        // Set default values if users exist
        if (adminUsers.length > 0) {
          setFormData(prev => ({
            ...prev,
            authorizedById: prev.authorizedById || adminUsers[0].id,
            accountManagerId: prev.accountManagerId || adminUsers[0].id,
            leadSourceId: prev.leadSourceId || adminUsers[0].id,
          }));
        }
        ...
        if (employeeUsers.length > 0) {
          setFormData(prev => ({
            ...prev,
            salesExecutiveId: prev.salesExecutiveId || employeeUsers[0].id,
          }));
        }
      }
    } catch (error) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto space-y-6">
          {/* Header */}
          <div>
            <h1 className="text-3xl font-bold">Create Master</h1>
            <p className="text-muted-foreground">
              Add a new customer or vendor master
            </p>
          </div>
